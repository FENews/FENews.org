...
> `yield` 关键字暂停 generator 函数执行，它后面的表达式的值作为 generator 的返回值。它也可以被认为是基于 generator 的 `return` 关键字。
...
> 一旦在 `yield` 表达式处暂停，generator 代码执行将保持暂停状态，直到调用 generator 的 `next()`。每次 `next()` 被调用，generator 都将恢复执行并返回 [iterator](https://codeburst.io/javascript-es6-iterables-and-iterators-de18b54f4d4) 结果。
...
每次 `UUID.next()` 都将返回一个新的 UUID 值，由于无限循环，done 始终为 false。
...
有很多 generators 的实现，很多库都有大量的使用，比如 [co](https://github.com/tj/co)，[koa](https://koajs.com/)，[redux-saga](https://github.com/redux-saga/redux-saga)。
